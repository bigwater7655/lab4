...
bool cache_set_c::evict_and_bring_new(int tag, bool set_dirty) {
  ...
  m_entry[i].m_tag = tag;
  bool was_dirty = m_entry[i].m_dirty;
...
bool cache_base_c::access(addr_t address, int access_type, bool is_fill) {
  ...
  if (access_type == 0 || access_type == 2) {
    ...
    if (!res) {
      ...
      if (m_set[set_idx]->evict_and_bring_new(tag, false)) m_num_writebacks++;
      m_num_misses++;
    }
    ...
    if (!res) {
      //write miss
     if (m_set[set_idx]->evict_and_bring_new(tag, true)) m_num_writebacks++;
     m_num_misses++;
    }
